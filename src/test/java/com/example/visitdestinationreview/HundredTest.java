...
public class HundredTest {
    ...
    private static final int DATA_COUNT = 1_000_000;
    ...
    public void generateVisitDestinationReviews() {
        String sql = "INSERT INTO visit_destination_reviews (user_id, destination_name, destination_description, " +
                "latitude, longitude, visit_date, review_content, rating, review_created_at) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        List<Object[]> batchArgs = new ArrayList<>();

        for (int i = 0; i < DATA_COUNT; i++) {
            System.out.println(i + "번째 리뷰 생성 중");
            long userId = random.nextInt(DATA_COUNT) + 1;
            ...
            batchArgs.add(new Object[]{userId, destinationName, description, latitude, longitude,
                    visitDate, reviewContent, rating, reviewCreatedAt});
